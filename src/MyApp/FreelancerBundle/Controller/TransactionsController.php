...
use MyApp\FreelancerBundle\Entity\Transactions;
...
class TransactionsController extends Controller{
    ...
    public function listTransactionFreelancerAction()
    {
        $em=$this->getDoctrine()->getEntityManager();
        $user = $this->container->get('security.context')->getToken()->getUser();
        $id=$user->getId();
        $listTransationFreelancer=$em->getRepository("FreelancerBundle:Transactions")
                ->listTransactionFreelancer($id);
        $montant=$em->getRepository("FreelancerBundle:Transactions")
                ->findMontantById($id);
        $listEmailJobowner=$em->getRepository("FreelancerBundle:Transactions")
        ...
        return $this->render("FreelancerBundle:Transactions:TransactionsFreelancer.html.twig", array("listTransactionByfreelancer" => $paginator,"idf"=>$id,"montant"=>$montant,"listEmailJobowner" => $listEmailJobowner));  
        }else{
           return $this->render("FreelancerBundle:Transactions:TransactionsFreelancer.html.twig", array("listTransactionByfreelancer" => $listTransationFreelancer,"idf"=>$id,"montant"=>$montant,"listEmailJobowner" => $listEmailJobowner)); 
        }
    ...
    public function searchTransactionFreelancerAction()
    {
        ...
        if($Request->getMethod()=="POST")
        {
            ...
            $date2 = $Request->get('date2');
            if($email != "tous")
            {$transaction=$em->getRepository("FreelancerBundle:Transactions")
            ->listTransactionFreelancerByEmailandDate($idf,$email,$date1,$date2);
            }else
            {$transaction=$em->getRepository("FreelancerBundle:Transactions")
            ->listTransactionFreelancerByDate($idf,$date1,$date2);
            }
            $listEmailFreelancer=$em->getRepository("FreelancerBundle:Transactions")
           ...
           return $this->render("FreelancerBundle:Transactions:TransactionsFreelancer.html.twig", array("listTransactionByfreelancer" => $paginator ,"listEmailJobowner" => $listEmailFreelancer,"idf"=>$idf,"montant"=>$montant));  
            }else{
                return $this->render("FreelancerBundle:Transactions:TransactionsFreelancer.html.twig", array("listTransactionByfreelancer" => $transaction ,"listEmailJobowner" => $listEmailFreelancer,"idf"=>$idf,"montant"=>$montant)); 
            }
